...
const QN = {
  ...
  MY_JOB: 132, // 나의 희망 직업
  PARENT_JOB: 133, // 부모님이 원하는 직업
  LIKE_SUBJ: 134, // 가장 좋아하는 과목 (limit=2 → "A,B")
  DISLIKE_SUBJ: 135, // 가장 싫어하는 과목 (limit=2 → "A,B")
...
// ‘결정함/결정못함’ → 커리어넷 값
const encodeDecision = (txt) => {
  const s = String(txt || "").replace(/\s+/g, "");
  if (s === "1" || s === "결정함") return "1";
  if (s === "2" || s === "결정못함") return "2";
  return "";
};
...
const InterestTestPage7 = () => {
  ...
  const [answers, setAnswers] = useState({
    ...
    2: [null, null, null], // [129 결정, 130 결정, 131 전공계열(라벨)]
    3: ["", ""], // [132, 133] 직업 ("1"~"62" 코드 저장)
    4: [
      ["", ""],
      ["", ""],
    ], // [134 좋아], [135 싫어] 각 2개 ("1"~"17" 코드 저장)
  ...
  useEffect(() => {
    const fetchPage7Questions = async () => {
      try {
        ...
        const arr1 = r1.result.questions;
        const map1 = [...arr1]
          .map((q) => ({
            no: Number(q.no ?? q.qno ?? q.id ?? 0),
            text: String(q.text ?? q.title ?? "").trim(),
          }))
        ...
        console.warn(
          "[page7] fetch 121~128 failed",
          e?.response?.status || e.message
        );
      }

      // 133(부모님이 원하는 직업)에서 choices 가져오기 (132와 동일 리스트)
      try {
        const rJob = await get(
          "https://www.career.go.kr/inspct/openapi/v2/test?apikey=69611c6585da333774ecf91966fc17f0&q=34 "
        );
        const qArr = rJob.result.questions;
        const q =
          qArr.find((x) => Number(x?.no ?? x?.qno ?? x?.id ?? 0) === 133) ??
          qArr[0];
        ...
        console.warn(
          "[page7] fetch job choices failed",
          e?.response?.status || e.message
        );
      }
      ...
      try {
        ...
        const arr2 = r2.result.questions;
        const map2 = [...arr2]
          .map((q) => ({
            no: Number(q.no ?? q.qno ?? q.id ?? 0),
            text: String(q.text ?? q.title ?? "").trim(),
  ...
  const handleSubjectClick = (rankIdx, code) => {
    ...
    if (otherUsed.has(code)) {
      alert(
        "이미 다른 문항에서 선택한 과목입니다. (4-1/4-2 전체에서 중복 불가)"
      );
      return;
    }
  ...
  const validatePage7 = () => {
      ...
      if (Array.isArray(arr[0])) {
        for (let sub = 0; sub < arr.length; sub++) {
          ...
          if (idx !== -1) {
            return {
              ok: false,
              msg: `${section}-${sub + 1}-${idx + 1}번 문항을 답해주세요.`,
              key: `${section}-${sub + 1}-${idx + 1}`,
            };
          }
        }
      } else {
    ...
    // 최종적으로 4칸 유니크 보증 (방어)
    const allPicked = [...answers[4][0], ...answers[4][1]].filter(Boolean);
    const uniq = new Set(allPicked);
    if (uniq.size !== allPicked.length) {
      return {
        ok: false,
        msg: "4-1과 4-2의 1·2순위 과목은 서로 중복될 수 없습니다.",
        key: "4-1-1",
      };
    }
  ...
  const buildV2Payload = () => {
    ...
    for (let i = 0; i < 8; i++) {
      arr.push({
        no: String(QN.SECTION1_START + i),
        val: String(answers[1][i]),
      });
    }
    ...
    for (let i = 0; i < 9; i++) {
      arr.push({
        no: String(QN.ACHIEVE_START + i),
        val: String(answers[6][i]),
      });
    }
    ...
    // 디버그: 131 확인
    const has131 = arr.some((x) => String(x.no) === "131");
    if (!has131) {
      console.error("DEBUG: 131 missing!", {
        dec2,
  ...
  const checkAndSubmit = async () => {
    ...
    if (!v.ok) {
      alert(v.msg);
      questionRefs.current[v.key]?.scrollIntoView({
        behavior: "smooth",
        block: "center",
      });
      return;
    ...
    try {
      ...
      // 간단 진단 로그
      {
        const nums = payload.answers.map((a) => Number(a.no));
        ...
        const missing = [];
        for (let n = 1; n <= 145; n++) if (!uniq.has(n)) missing.push(n);
        const dups = [];
        const seen = new Set();
        for (const n of nums) {
        ...
        console.log("[DIAG] missing ->", missing);
        console.log("[DIAG] dups    ->", dups);
        console.log(
          "[CHECK] has131",
          Array.isArray(payload.answers) &&
            payload.answers.some((a) => String(a.no) === "131"),
          Array.isArray(payload.answers) &&
            payload.answers.find((a) => String(a.no) === "131")
        );
        console.log("[FINAL peek]", {
          132: payload.answers.find((a) => a.no === "132"),
          133: payload.answers.find((a) => a.no === "133"),
          134: payload.answers.find((a) => a.no === "134"),
          135: payload.answers.find((a) => a.no === "135"),
          136: payload.answers.find((a) => a.no === "136"),
          131: payload.answers.find((a) => a.no === "131"),
          count: payload.answers.length,
          trgetSe: payload.trgetSe,
    ...
    try {
      // 실제 전송 (디버그 문자열 제거)
      payload.apikey = "69611c6585da333774ecf91966fc17f0";
      ...
      // 결과 URL 처리
      const urlFromApi =
        res?.data?.result?.inspct?.reporturl || res?.data?.RESULT?.url || "";

      let finalUrl = urlFromApi;
      if (!finalUrl) {
        const seq =
          res?.data?.result?.inspct?.inspctseq || res?.data?.RESULT?.inspctSeq;
        if (seq) {
    ...
    <div className="interest7-container">
      ...
      <div className="questions-wrapper">
        ...
        <div className="section">
          ...
          {Array.from({ length: 8 }, (_, i) => (
            <div key={i}>
              <div className="question-text">{`1-${i + 1}. ${
                sec1Texts[i]
              }`}</div>
                ...
                {[1, 2, 3, 4, 5].map((val, j) => (
                  <div key={val} className="circle-container-7">
                    <div
                      ...
                      className={`circle-7 ${
                        String(answers[1][i]) === String(val) ? "selected" : ""
                      }`}
                      onClick={() => handleSelect(1, i, String(val))}
                    />
                    {j === 0 && (
                      <div className="circle-label-7">〈 매우 그렇지 않다</div>
                    )}
                    {j === 2 && <div className="circle-label-7">보통</div>}
                    {j === 4 && (
                      <div className="circle-label-7">매우 그렇다 〉</div>
                    )}
                  </div>
                ))}
              </div>
        ...
        <div className="section">
          ...
          <div>
            ...
            {encodeDecision(answers[2][1]) === "2" && (
              <p className="small-guide" style={{ marginTop: 8 }}>
                ※ 2-2에서 ‘결정못함’을 선택하면 이 항목은 건너뜁니다.
              </p>
            )}
          </div>
        </div>
          ...
          {["나의 희망 직업", "부모님이 원하는 직업"].map((label, idx) => (
            <div key={idx}>
              ...
              <div className="job-select-7">
                <div
                  className="job-box-7"
                  onClick={() => setShowJobModal({ open: true, index: idx })}
                >
                  {answers[3][idx]
                    ? jobCodeToText(answers[3][idx])
                    : `직업을 선택해주세요.`}
                </div>
                <button
                  className="page7button"
                  onClick={() => setShowJobModal({ open: true, index: idx })}
                >
                  직업 선택
                </button>
        ...
        <div className="section">
          ...
          {["가장 좋아하는 과목", "가장 싫어하는 과목"].map(
            (label, sectionIdx) => (
              <div key={sectionIdx}>
                <div className="question-text">{`4-${
                  sectionIdx + 1
                }. ${label}`}</div>
            ...
            <div className="circle-options-7">
              {[1, 2, 3, 4, 5].map((val, j) => (
                <div key={val} className="circle-container-7">
                  <div
                    className={`circle-7 ${
                      String(answers[5][0]) === String(val) ? "selected" : ""
                    }`}
                    onClick={() => handleSelect(5, 0, String(val))}
                  />
                  {j === 0 && (
                    <div className="circle-label-7">〈 매우 불만족</div>
                  )}
                  {j === 2 && <div className="circle-label-7">보통</div>}
                  {j === 4 && (
                    <div className="circle-label-7">매우 만족 〉</div>
                  )}
                </div>
              ))}
            </div>
        ...
        <div className="section">
          <div className="section-title">
            6. 자신의 학업성적이 과목별로 대략 어느 범위에 속한다고
            생각하십니까?
          </div>
          {Array.from({ length: 9 }, (_, i) => (
            <div key={i}>
              <div className="question-text">{`6-${i + 1}. ${
                achieveLabels[i]
              }`}</div>
              <div className="circle-options-7">
                {ACHIEVE_CHOICES.map((opt, j) => (
                  <div key={opt.val} className="circle-container-7">
                    ...
                    {j === 5 && (
                      <div className="circle-label-7">배우지 않음</div>
                    )}
                  </div>
                ))}
              </div>
        ...
        <div className="button-group">
          ...
          <button className="next-button" onClick={checkAndSubmit}>
            제출하기
          </button>
        </div>
      </div>
      ...
      {showJobModal.open && (
        <div className="modal" role="dialog" aria-modal="true">
          <div
            className="modal-content modal-square job-modal"
          ...
          >
            ...
            <h4>직업 선택</h4>
            <div
              className="modal-list modal-grid-tight"
            ...
            >
              {jobChoices.map((job) => (
                <button
                  ...
                  onClick={() => {
                    const updated = { ...answers };
                    updated[3][showJobModal.index] = job.val; // "1"~"62" 코드 저장
                    setAnswers(updated);
      ...
      {showSubjectModal.open && (
        <div className="modal" role="dialog" aria-modal="true">
          <div
            className="modal-content modal-square"
            style={{ maxHeight: "80vh", overflowY: "auto" }}
          >
            <button
              className="modal-x"
              ...
              >
                {SUBJECT_CHOICES.map((subj) => (
                  <button
                    key={`s1-${subj.val}`}
                    className={`modal-btn ${
                      tempSubjects[0] === subj.val ? "selected" : ""
                    }`}
              ...
              >
                {SUBJECT_CHOICES.map((subj) => (
                  <button
                    key={`s2-${subj.val}`}
                    className={`modal-btn ${
                      tempSubjects[1] === subj.val ? "selected" : ""
                    }`}
