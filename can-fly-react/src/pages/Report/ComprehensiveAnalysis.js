...
/**
 * props:
 *  - text: 문자열(멀티라인) 또는 문자열 배열
 *  - lines: 배열로 바로 줄 때
 *  - fallback: 데이터 없을 때 대체 배열
 */
export default function ComprehensiveAnalysis({ text, lines, fallback = DEFAULT_FALLBACK }) {
  const analysisText = useMemo(() => {
    if (Array.isArray(lines) && lines.length) return lines;
    if (Array.isArray(text)) return text.length ? text : fallback;
